...
namespace osu.Game.Rulesets.Osu.Replays
{
    ...
    {
        public new KeijoBeatmap Beatmap => (KeijoBeatmap)base.Beatmap;
        ...
        public KeijoAutoGenerator(IBeatmap beatmap)
            : base(beatmap)
        {
        ...
        public override Replay Generate()
        {
            ...
            AddFrameToReplay(new KeijoReplayFrame(-100000, new Vector2(256, 500)));
            AddFrameToReplay(new KeijoReplayFrame(Beatmap.HitObjects[0].StartTime - 1500, new Vector2(256, 500)));
            AddFrameToReplay(new KeijoReplayFrame(Beatmap.HitObjects[0].StartTime - 1500, new Vector2(256, 500)));

            for (int i = 0; i < Beatmap.HitObjects.Count; i++)
            {
                KeijoHitObject h = Beatmap.HitObjects[i];

                if (DelayedMovements && i > 0)
                {
                    KeijoHitObject prev = Beatmap.HitObjects[i - 1];
        ...
        private void addDelayedMovements(KeijoHitObject h, KeijoHitObject prev)
        {
            ...
            {
                if (!(prev is Spinner) && h.StartTime - endTime < 1000) AddFrameToReplay(new KeijoReplayFrame(endTime + h.HitWindows.HalfWindowFor(HitResult.Meh), new Vector2(prev.StackedEndPosition.X, prev.StackedEndPosition.Y)));
                if (!(h is Spinner)) AddFrameToReplay(new KeijoReplayFrame(h.StartTime - h.HitWindows.HalfWindowFor(HitResult.Miss), new Vector2(h.StackedPosition.X, h.StackedPosition.Y)));
            }
            else if (h.StartTime - h.HitWindows.HalfWindowFor(HitResult.Meh) > endTime + h.HitWindows.HalfWindowFor(HitResult.Meh) + 50)
            {
                if (!(prev is Spinner) && h.StartTime - endTime < 1000) AddFrameToReplay(new KeijoReplayFrame(endTime + h.HitWindows.HalfWindowFor(HitResult.Meh), new Vector2(prev.StackedEndPosition.X, prev.StackedEndPosition.Y)));
                if (!(h is Spinner)) AddFrameToReplay(new KeijoReplayFrame(h.StartTime - h.HitWindows.HalfWindowFor(HitResult.Meh), new Vector2(h.StackedPosition.X, h.StackedPosition.Y)));
            }
            else if (h.StartTime - h.HitWindows.HalfWindowFor(HitResult.Good) > endTime + h.HitWindows.HalfWindowFor(HitResult.Good) + 50)
            {
                if (!(prev is Spinner) && h.StartTime - endTime < 1000) AddFrameToReplay(new KeijoReplayFrame(endTime + h.HitWindows.HalfWindowFor(HitResult.Good), new Vector2(prev.StackedEndPosition.X, prev.StackedEndPosition.Y)));
                if (!(h is Spinner)) AddFrameToReplay(new KeijoReplayFrame(h.StartTime - h.HitWindows.HalfWindowFor(HitResult.Good), new Vector2(h.StackedPosition.X, h.StackedPosition.Y)));
            }
        }

        private void addHitObjectReplay(KeijoHitObject h)
        {
            ...
            if (h is Spinner)
            {
                calcSpinnerStartPosAndDirection(((KeijoReplayFrame)Frames[Frames.Count - 1]).Position, out startPosition, out spinnerDirection);

                Vector2 spinCentreOffset = SPINNER_CENTRE - ((KeijoReplayFrame)Frames[Frames.Count - 1]).Position;
        ...
        private void moveToHitObject(KeijoHitObject h, Vector2 targetPos, Easing easing)
        {
            KeijoReplayFrame lastFrame = (KeijoReplayFrame)Frames[Frames.Count - 1];
            ...
            {
                lastFrame = new KeijoReplayFrame(waitTime, lastFrame.Position) { Actions = lastFrame.Actions };
                 ...
                 timeDifference >= 266)) // ... or the beats are slow enough to tap anyway.
            {
                ...
                for (double time = lastFrame.Time + FrameDelay; time < h.StartTime; time += FrameDelay)
                {
                    ...
                    AddFrameToReplay(new KeijoReplayFrame((int)time, new Vector2(currentPosition.X, currentPosition.Y)) { Actions = lastFrame.Actions });
                }
        ...
        // Add frames to click the hitobject
        private void addHitObjectClickFrames(KeijoHitObject h, Vector2 startPosition, float spinnerDirection)
        {
            ...
            var endFrame = new KeijoReplayFrame(hEndTime + endDelay, new Vector2(h.StackedEndPosition.X, h.StackedEndPosition.Y));
            ...
            if (index >= 0)
            {
                var previousFrame = (KeijoReplayFrame)Frames[index];
                ...
                if (previousActions.Any())
                {
                    ...
                    {
                        var frame = (KeijoReplayFrame)Frames[j];
            ...
            switch (h)
            {
                ...
                case Spinner spinner:
                    ...
                    for (double j = h.StartTime + FrameDelay; j < spinner.EndTime; j += FrameDelay)
                    {
                        ...
                        AddFrameToReplay(new KeijoReplayFrame((int)j, new Vector2(pos.X, pos.Y), action));
                    }

                    t = ApplyModsToTime(spinner.EndTime - h.StartTime) * spinnerDirection;
                    Vector2 endPosition = SPINNER_CENTRE + CirclePosition(t / 20 + angle, SPIN_RADIUS);

                    AddFrameToReplay(new KeijoReplayFrame(spinner.EndTime, new Vector2(endPosition.X, endPosition.Y), action));
                ...
                case Slider slider:
                    for (double j = FrameDelay; j < slider.Duration; j += FrameDelay)
                    {
                        ...
                        AddFrameToReplay(new KeijoReplayFrame(h.StartTime + j, new Vector2(pos.X, pos.Y), action));
                    }

                    AddFrameToReplay(new KeijoReplayFrame(slider.EndTime, new Vector2(slider.StackedEndPosition.X, slider.StackedEndPosition.Y), action));
                    break;
            }
