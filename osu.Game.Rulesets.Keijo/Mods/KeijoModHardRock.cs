...
namespace osu.Game.Rulesets.Keijo.Mods
{
    public class KeijoModHardRock : ModHardRock, IApplicableToHitObject
    {
        ...
        {
            var osuObject = (KeijoHitObject)hitObject;

            osuObject.Position = new Vector2(osuObject.Position.X, KeijoPlayfield.BASE_SIZE.Y - osuObject.Y);

            var slider = hitObject as Slider;
            if (slider == null)
                return;

            slider.NestedHitObjects.OfType<SliderTick>().ForEach(h => h.Position = new Vector2(h.Position.X, KeijoPlayfield.BASE_SIZE.Y - h.Position.Y));
            slider.NestedHitObjects.OfType<RepeatPoint>().ForEach(h => h.Position = new Vector2(h.Position.X, KeijoPlayfield.BASE_SIZE.Y - h.Position.Y));
