...
namespace osu.Game.Rulesets.Keijo.Mods
{
    public class KeijoModHidden : ModHidden
    {
        ...
        {
            void adjustFadeIn(KeijoHitObject h) => h.TimeFadeIn = h.TimePreempt * fade_in_duration_multiplier;

            foreach (var d in drawables.OfType<DrawableKeijoHitObject>())
            {
                adjustFadeIn(d.HitObject);
                foreach (var h in d.HitObject.NestedHitObjects.OfType<KeijoHitObject>())
                    adjustFadeIn(h);
            }
        ...
        {
            if (!(drawable is DrawableKeijoHitObject d))
                return;
